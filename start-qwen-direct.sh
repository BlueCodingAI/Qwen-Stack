...
TEMPLATE="${QWEN_TEMPLATE:-ad7f44ce435d59f8dfd2a16af201ff37}"   # image only; the model comes
                                            # from onstart-direct.sh
LABEL="${QWEN_LABEL:-qwen-direct}"          # how the stop script recognises our instance
DISK="${QWEN_DISK:-160}"                    # GB; the weights are ~56 GB
...
if (( FRESH )); then
    warn "     a fresh instance pulls the image and 56 GB of weights first - up to ~15 min"
